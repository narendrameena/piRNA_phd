...
def extract_lncRNA_features_from_gff3(input_gff3, output_gff3):
    features = {}
    lncRNA_regions = []
    ...
    with open(input_gff3, 'r') as file:
        for line in file:
            if not line.startswith("#"):
                ...
                # Recording lncRNA regions
                if feature_type == "lnc_RNA":
                    lncRNA_regions.append((chrom, int(start), int(end)))
                
                if feature_type in ["exon", "three_prime_UTR", "five_prime_UTR"]:
    ...
    with open(output_gff3, 'w') as output:
        ...
        for feature_type, entries in features.items():
            for entry in entries:
                ...
                # Check if the feature lies within lncRNA regions
                in_region = any(s <= start <= e and s <= end <= e for c, s, e in lncRNA_regions if c == chrom)
                
                if in_region:
                    output.write(f"{chrom}\tcustom\t{feature_type}\t{start}\t{end}\t.\t{strand}\t.\t{attributes}\n")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Extract exons, UTRs, and introns exclusively for lnc_RNA from a GFF3 file and modify ID attributes.')
    parser.add_argument('input_gff3', help='Input GFF3 file')
    parser.add_argument('output_gff3', help='Output GFF3 file with modified features for lnc_RNA only')
    args = parser.parse_args()
    
    extract_lncRNA_features_from_gff3(args.input_gff3, args.output_gff3)
