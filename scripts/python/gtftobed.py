...
if len(sys.argv) < 2:
    ...
    print(
        '2\tIn the attribute list of .GTF file, the script tries to find "gene_id", "transcript_id" and "FPKM" attribute, and convert them as name and score field in .BED file.')
...
def printbedline(estart, eend, field, nline):
    try:
        ...
        print(
            field[0] + '\t' + str(estp) + '\t' + str(eedp) + '\t' + transid + '\t' + str(fpkmint) + '\t' + field[
                6] + '\t' + str(estp) + '\t' + str(eedp) + '\t' + color + '\t' + str(len(estart)) + '\t', end='')
        ...
        for i in range(1, len(segstart)):
            strs += ',' + str(segstart[i])
        print(strl + '\t' + strs + '\t' + transid)  # Add transid as the last column in the BED file
    except ValueError:
...
for lines in open(sys.argv[-1]):
    ...
    nline = nline + 1
    if len(field) < 9:
        print('Error: the GTF should have at least 9 fields at line ' + str(nline), file=sys.stderr)
        continue
