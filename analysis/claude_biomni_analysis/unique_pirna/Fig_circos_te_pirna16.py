...
family (L1/ERVK/ERVL) so they compare family-by-family. For each strain × timepoint:
  * OUTER half-band = active-TE EXPRESSION (small-RNA sense-to-TE on TEs, grows OUTWARD) = threat
...
TEFAM=[("L1","#1B7837"),("ERVK","#5AAE61"),("ERVL","#A6DBA0")]   # TE expression (SENSE-to-TE small RNA) = GREEN shades (L1→ERVK→ERVL)
PIFAM=[("L1","#762A83"),("ERVK","#9970AB"),("ERVL","#C2A5CF")]   # piRNA response (ANTISENSE-to-TE small RNA) = PURPLE shades
clen={}
for ln in open(FAI):
    f=ln.split("\t")
...
def load(fn):
    ...
    return D
TE=load(f"{U}/active_te_expression_sRNA_tp.tsv"); PI=load(f"{U}/active_pirna_on_te_sRNA_tp.tsv")   # sRNA-only: sense-to-TE=expression, antisense-to-TE=piRNA (no small-RNA sense-to-TE, no liftover)
...
leg=[Line2D([0],[0],color="#1B7837",lw=7,label="TE threat: L1"),Line2D([0],[0],color="#5AAE61",lw=7,label="TE: ERVK"),Line2D([0],[0],color="#A6DBA0",lw=7,label="TE: ERVL"),
     Line2D([0],[0],color="#762A83",lw=7,label="piRNA resp: L1"),Line2D([0],[0],color="#9970AB",lw=7,label="piRNA: ERVK"),Line2D([0],[0],color="#C2A5CF",lw=7,label="piRNA: ERVL")]
fig.legend(handles=leg,loc="lower center",bbox_to_anchor=(0.5,0.045),ncol=3,fontsize=11,frameon=False,title="GREEN = active-TE EXPRESSION (sense-to-TE small RNA, threat, outward) · PURPLE = piRNA-on-TE (response, inward); shade = family (dark→light: L1→ERVK→ERVL); per strain × 3 timepoints; HEIGHT ∝ log reads",title_fontsize=9.5)
fig.suptitle("TE-SILENCING ARMS-RACE circos — the transposon threat vs the piRNA counter-strike, family-by-family across development, 16 strains × 3 timepoints in one GRCm39 circle\n"
             "GREEN = active-TE EXPRESSION firing OUTWARD (threat) · PURPLE = piRNA-on-TE striking INWARD (response); shade = family (L1/ERVK/ERVL); tall green meeting tall purple of the SAME family = the ping-pong cycle caught in the act",
...
_pd.DataFrame([(X,tp,bins[bi][0],bins[bi][1],fam,round(v,3)) for (X,tp),_bd in TE.items() for bi,_fd in _bd.items() for fam,v in _fd.items()],columns=["strain","timepoint","chrom","bin_start","TE_family","sRNA_expression"]).to_csv(f"{_SD}/SourceData_Fig_circos_te_pirna16_TE.csv",index=False)
_pd.DataFrame([(X,tp,bins[bi][0],bins[bi][1],fam,round(v,3)) for (X,tp),_bd in PI.items() for bi,_fd in _bd.items() for fam,v in _fd.items()],columns=["strain","timepoint","chrom","bin_start","TE_family","piRNA_on_TE"]).to_csv(f"{_SD}/SourceData_Fig_circos_te_pirna16_piRNA.csv",index=False)
