#!/bin/bash
# Orchestrate the within-timepoint SNP refinement (produces theme-18 deseq-stagepeak input);
# NOTE: the klass5 re-derivation step (within_tp_rederive.py) was REMOVED — it clobbered the 5-class final.
...
#   (step 5 within_tp_rederive REMOVED — it overwrote the 5-class final_classified_clean_2read.csv.gz)
set -uo pipefail
...
# 5. REMOVED: within_tp_rederive.py overwrote final_classified_clean_2read.csv.gz with a buggy 4-class version
#    (its bitwise `& ~sb` dropped the 'expressed elsewhere (exact)' class). The 5-class final is OWNED by
#    make_klass5.py — do NOT re-add this step. Steps 1-4 above still produce snp_variant_refinement_withintp.csv,
#    which theme 18 (classify_deseq_stagepeak.py) reads. ee_withintp_diag.py (also a theme-18 input) is separate.
