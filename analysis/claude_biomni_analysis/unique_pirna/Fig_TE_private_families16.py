...
# featureCounts Geneid = "<name> <class/family> <chrom> <start-end>" -> family is the 2nd whitespace token
famexpr=defaultdict(lambda: defaultdict(float))
for X in CANON:
    fc=f"{ROOT}/results/TE_expression_rna/{X}.TE.featureCounts"
    ...
    for ln in open(fc):
        ...
        try: famexpr[parts[1]][X]+=sum(float(x) for x in c[6:])
...
# Panel C — RNA-seq TE-family expression (same families x strains)
_Elog=np.log10(E.values+1); _cmapE=plt.get_cmap("viridis").copy(); _cmapE.set_bad("white")
...
axE.set_title("C  TE-family RNA-seq EXPRESSION (same families × strains; cell = summed RNA-seq counts, colour = log)",fontsize=9.6,fontweight="bold",loc="left")
cbE=fig.colorbar(imE,ax=axE,fraction=0.025,pad=0.01); cbE.set_label("log10(RNA count +1)",fontsize=7); cbE.ax.tick_params(labelsize=6)
...
fig.suptitle("TE families driving strain-private piRNAs across 16 strains — piRNA origin (A) vs RNA-seq TE expression (C); LTR/ERVK(IAP) + LINE/L1 dominant, more so in wild-derived strains",fontsize=10.2,fontweight="bold",y=1.0)
