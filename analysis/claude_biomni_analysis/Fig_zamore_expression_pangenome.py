...
# Okabe-Ito sequential warm gradient (developmental order) -- project NG palette
...
for j,t in enumerate(TPO):
    ...
    for xi,v,c in zip(xs, piv[t].values, cnt[t].values):   # number of loci atop each bar
        axA.text(xi, v+1.0, str(int(c)), rotation=90, ha="center", va="bottom",
                 fontsize=4.2, color="#333", zorder=4)
axA.set_xticks(x)
...
axA.set_ylim(0,113); axA.set_yticks([0,20,40,60,80,100]); axA.set_xlim(-0.6,n-0.4)
...
fig.text(0.5,-0.04,
    f"n={n_tot} Zamore pachytene piRNA loci (Li 2013 Mol Cell PMID 23523368; mm10→mm39 UCSC liftOver) · number atop each bar = loci expressed (of {n_tot}) · "
    "expression = locus overlaps strain's combined-run PICB clusters · cross-strain projection = cactus halLiftover (pangenome) · "
    "red strain label = wild-derived",
