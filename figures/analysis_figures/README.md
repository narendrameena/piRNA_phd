...
| **04_picb_method_qc** | `Fig_PICB_chr_vs_wholebam_comparison`, `Fig_PICB_chrbychr_vs_wholebam`, `mapping_complexity/01-12 + report` | `picb_chr_vs_whole_bam_comparison.py`, `picb_comparison_figure.py` | results/picb_result + picb_result_combined |
...
| 16 | trinity_precursors | 1 | ✓ **NEW 2026-06-21** (thesis Ch.6; de-novo precursors, coverage rule = thesis **100 RPM & 100 RPKM** = saved all_ data, 546,483 precursors; committed `.sh` column bug flagged) |
| 17 | picb_vs_trinity | 5 | ✓ **NEW 2026-06-21** (PICB vs Trinity head-to-head + 2 method-test figs; build-compat verified; PICB 3–7× more loci, recovers <12% by Trinity, Trinity over-segments 2.6–5.8×; piRNA read-capture PICB ≥ Trinity — tied at pachytene, ~2× at P12.5 → PICB better for source loci, Trinity complementary; BioMNI 2/3 confirm; **+read-budget fig 2026-06-23**: total sRNA/piRNA + intron-included Trinity bar, intron inflation ×1.05–1.6) |
| 18 | deseq2_stagepeak_unique_piRNA | 10 | ✓ **NEW 2026-06-23** (additive; theme 07 untouched). DESeq2 ADOPTED over edgeR (benchmark: edgeR ~10–21× more null FP, ~2× anti-conservative; BioMNI 3/3). 27/30 nt stage-peak filter BEFORE DESeq2 (+6–7% vs after). STRICT WITHIN-TP uniqueness (BioMNI 3/3): **10,724 genuinely-unique (67%)** via 3 mechanisms — insertion / regulatory (conserved-but-silent) / **heterochronic (stage-shifted)**; SNP-refinement removes 29% strain alleles; PCA wild strains separate (SPRET dominates). **TE-origin (BioMNI 3/3): stage-shifted 45% TE-derived (ERVK 39%/L1 18%) ≈ insertion-driven 50% (ERVK 45%/L1 17%) > regulatory 31% → heterochronic uniqueness is young-TE-driven. Sense/antisense (BioMNI 3/3): stage-shifted 56% + CBS 57% antisense-to-TE = silencing-competent, vs strain-private 47% (~50/50 dual-strand new insertions) → stage-shifted = HYBRID (young-TE origin like insertion + antisense silencing like conserved).** Citations PMID+DOI verified |
| 19 | exact_vs_snp_uniqueness | 4 | ✓ **NEW 2026-06-23** (additive; +Theme-18-style heatmap & PCA on the exact set). EXACT-sequence vs SNP-aware definition of strain-unique piRNAs. EXACT (1–3 SNP variants kept) = 15,118 vs SNP-aware (Theme 18) = 10,724 (+41%). TEST: the +4,394 SNP-alleles are shared (±1–3 SNP) with **mean 11.1 of 15 other strains** (peak at all 15), 88% single-SNP → STANDING GENETIC VARIATION, not innovation. **BioMNI 3/3: SNP-aware preferred** (separates innovation from standing variation). |
