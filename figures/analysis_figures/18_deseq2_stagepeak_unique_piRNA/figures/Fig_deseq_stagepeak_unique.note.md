...
- **Caveat (reproducibility, not direction):** the "same-stage allele" (SNP-variant) flag is gated by the pangenome ortholog projection (`present_in_{Y}.bed`, halLiftover over the cactus HAL) and a forward-strand expressed-pool comparison, so a minus-strand-only or unprojected same-stage homolog can be missed — modestly *inflating* genuinely-unique / strain-private in the most divergent (wild) strains. The wild-dominance direction is robust; the exact counts are a slight upper bound.
