#!/usr/bin/env python3
"""TEST FIGURE — justifies the ADOPTED >=2-read absence threshold for strain-specific piRNA calling.
Across the absence ladder (0-tolerance/strict -> >=2-read adopted -> loose), at the (moot) >=2/3-rep
presence rule, shows (A) pooled strain-specific count and (B) per-(strain x timepoint) read-mass coverage
...
# Panel A — pooled strain-specific count
...
axA.set_ylabel("strain-specific piRNAs (Σ over 16 strains × 3 tp)",fontsize=9)
...
axB.set_ylabel("strain-specific read-mass coverage\n(% of strain's total piRNA expression)",fontsize=9)
...
fig.suptitle("Choosing the absence threshold for strain-specific piRNAs — the adopted ≥2-read rule",fontsize=11,fontweight="bold",y=1.0)
fig.text(0.5,-0.05,"0-tolerance loses real strain-specific piRNAs (a single index-hopping read elsewhere disqualifies a well-expressed locus): "
...
src=pd.DataFrame({"absence_threshold":["strict_0tolerance","2read_ADOPTED","loose"],
                  "sum_strain_specific":counts,"mean_readmass_coverage_pct":[round(m,4) for m in means],
