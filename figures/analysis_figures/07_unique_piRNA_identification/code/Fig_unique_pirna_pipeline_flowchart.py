...
Counts = the adopted >=2-read, 25-32 nt set (final_classified_clean_2read.csv.gz), read live so the schematic never goes stale."""
...
# --- data-driven klass5 counts (read live so every displayed count is current, never stale) ---
import pandas as _pd, os as _os
_KP="/mnt/home3/miska/nm667/scratch/inProgress/mice_PiRNA/analysis/claude_biomni_analysis/unique_pirna/unique16/final_classified_clean_2read.csv.gz"
_vc=_pd.read_csv(_KP,usecols=["klass5"]).klass5.value_counts()
EX=int(_vc.get("expressed elsewhere (exact)",0)); SNP=int(_vc.get("SNP-variant (1-3mm)",0)); LOWQ=int(_vc.get("low-quality: no mm0 own-genome locus",0))
CBS=int(_vc.get("unique: conserved-but-silent",0)); SP=int(_vc.get("unique: strain-private locus",0)); TOT=int(_vc.sum())
NOTU=EX+SNP+LOWQ; GEN=CBS+SP; pNOTU=round(100*NOTU/TOT); pGEN=round(100*GEN/TOT)
...
ax.text(50,96.2,f"parameters in italics · counts = adopted ≥2-read, 25–32 nt set ({TOT:,} candidates, 16 strains)",ha="center",fontsize=8.5,color="#666")
...
arr(50,71.7,50,70.7)
box(50,68.5,40,4.2,f"strain-specific candidate piRNAs\n{TOT:,}","#fde9d9",fs=9.5,bold=True)
...
box(90,55,17,5.0,f"expressed-\nelsewhere (exact)\n{EX:,}","#e8e8e8",ec=GREY,tc="#333",fs=8,bold=True)
...
yL=29
box(16,yL,20,5.2,f"low-quality\nno mm0 own locus\n{LOWQ:,}",TAN,ec="#9a7b3b",tc="#3a2d10",fs=8.2,bold=True)
box(40,yL,20,5.2,f"UNIQUE:\nstrain-private locus\n{SP:,}","#efe3f6",ec=PURP,tc=PURP,fs=8.2,bold=True)
box(56,yL,20,5.2,f"SNP-variant\n(1–3 mm allelic)\n{SNP:,}","#fbeccd",ec=ORANGE,tc="#7a5500",fs=8.2,bold=True)
box(80,yL,20,5.2,f"UNIQUE:\nconserved-but-silent\n{CBS:,}","#dcebf6",ec=BLUE,tc=BLUE,fs=8.2,bold=True)
...
ax.text(28,16.2,f"NOT UNIQUE — {NOTU:,} ({pNOTU}%)",ha="center",fontsize=10,fontweight="bold",color="#555")
ax.text(28,12.5,f"expressed-elsewhere ({EX:,}) · SNP-variant ({SNP:,})\n· low-quality ({LOWQ:,})",ha="center",fontsize=8,color="#666",linespacing=1.3)
...
ax.text(73,16.2,f"GENUINELY UNIQUE — {GEN:,} ({pGEN}%)",ha="center",fontsize=10,fontweight="bold",color=PURP)
ax.text(73,12.5,f"unique: conserved-but-silent ({CBS:,}, divergence-driven)\n+ unique: strain-private locus ({SP:,}, insertion-driven)",ha="center",fontsize=8,color="#444",linespacing=1.3)
...
_vc.rename_axis("klass5").reset_index(name="n_candidates").to_csv(f"{_SD}/SourceData_Fig_unique_pirna_pipeline_flowchart.csv",index=False)   # CURRENT klass5 counts — now ALSO drive every schematic box above (data-driven, cannot go stale)
