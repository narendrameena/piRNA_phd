...
d=pd.read_csv(f"{U}/unique16/final_classified_clean_2read.csv.gz")   # ADOPTED ≥2-read absence (5-class klass5; genuinely-unique classes identical to loose, expressed-elsewhere trimmed)
...
def _wildmark(ax,span=True):
    "colour wild strain x-labels red + shade the wild-derived block (intuitive strain classification)"
...
ax1.set_ylabel("strain-specific piRNA candidates (count, log)",fontsize=9)
ax1.legend(fontsize=7,frameon=False,ncol=5,loc="lower left",bbox_to_anchor=(0,1.005),columnspacing=1.0,handlelength=1.3)
ax1.set_title("A  16-strain pangenome-syntenic unique-piRNA classification (mm0-clean strain-private) — counts per strain, canonical order",fontsize=10.4,fontweight="bold",pad=30)
_wildmark(ax1); ax1.text(np.mean(WPOS),ct.values.max()*2.0,"wild-derived",ha="center",va="top",fontsize=7.5,fontweight="bold",color="#C0392B")
...
ax3.set_ylabel("timepoint origin\n(proportion of genuinely-unique)",fontsize=9)
ax3.legend(fontsize=7.5,frameon=False,ncol=3,loc="lower center",bbox_to_anchor=(0.5,1.045),columnspacing=1.6)
ax3.set_title("C  Developmental timepoint origin of genuinely-unique piRNAs — per strain (strain-wise × timepoint-wise, NO pooling); text = n",fontsize=10,fontweight="bold",pad=34)
for xi,s in zip(x,CANON): ax3.text(xi,1.01,f"{int(tpc.loc[s,list(TPC)].sum()):,}",ha="center",va="bottom",fontsize=4.8,color="#777")
_wildmark(ax3); ax3.spines[['top','right']].set_visible(False)
...
ax4.legend(fontsize=7.5,frameon=False,ncol=3,loc="lower center",bbox_to_anchor=(0.5,1.045),columnspacing=1.6)
ax4.set_title("D  Developmental timepoint origin BY CLASS (pooled over 16 strains); text = n",fontsize=10,fontweight="bold",pad=30)
