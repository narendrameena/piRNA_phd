...
df["fold"]=df["fold_uniqmap"]   # HEADLINE = NH==1 UNIQUELY-MAPPING fold: the clean, un-inflatable signal (a unique read's single locus is an unambiguous production site). NH>1 multi-mappers are un-localizable (map to private+ancestral copies alike, see Fig_uniquelymapping_rationale) -> their apparent enrichment = maps-everywhere artifact. The inclusive matched-null-on-all `fold_matched`, the naive `fold`, and per-class NH counts are all retained in the CSV.
...
ax.set_ylabel("fold-enrichment: production locus inside a 1-of-16-private\ninsertion — UNIQUELY-MAPPING piRNAs (obs ÷ genomic null)",fontsize=9)
...
fig.text(0.5,0.005,"Numbers on bars = fold-enrichment of UNIQUELY-MAPPING (NH==1) piRNAs over the genomic null (% under each strain = merged private-insertion fraction of genome). With the CLEAN klass5 strain-private set, strain-private-locus piRNAs (pangenome: locus absent in all 15 other strains) are the MOST insertion-enriched class — the single locus a uniquely-mapping strain-private piRNA is transcribed from lies inside a private insertion ~52-70% of the time in divergent strains, vs ~5% for common piRNAs: a clean, un-inflatable signal. Multi-mapping (NH>1) piRNAs are EXCLUDED here because they map to the private insertion AND its ancestral TE copies alike, so their production locus is un-localizable (see Fig_uniquelymapping_rationale); the inclusive multiplicity-matched-null estimate on ALL candidates + the naive fold are retained in the source data. [Correction: the earlier 3-class estimate (~24% in-insertion) was DILUTED by mm1-3 low-quality reads lacking a clean mm0 locus; klass5 removes them.] The class is still a minority of candidates (klass5 strain-private = "+f"{_sp:,} = {_sppct:.1f}%"+"). "
