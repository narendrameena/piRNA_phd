...
R_OUT,R_IN=0.95,0.17; grp_h=(R_OUT-R_IN)/len(CANON); gap_g=grp_h*0.12; cons_h=grp_h*0.18; sub_h=(grp_h-gap_g-cons_h)/3
for k,X in enumerate(CANON):
    top=R_OUT-k*grp_h
    occ=set()                                            # per-strain CONSERVATION layer (top of each strain group):
    for _,tp in TPS: occ|=set(CL[(X,tp)].keys())         # this strain's clusters shaded by # of 16 strains sharing the bin
    rc=top-gap_g-cons_h; oc=sorted(occ)
    if oc: ax.bar(bth[oc],height=cons_h,width=bwd[oc],bottom=rc,color=[CMAP(CNORM(cons[b])) for b in oc],align="center",edgecolor="none",linewidth=0,rasterized=True)
    for j,(lab,tp) in enumerate(TPS):
        r=rc-(j+1)*sub_h; d=CL[(X,tp)]
        if d:
            bb=sorted(d)
