...
if os.path.exists(E_csv):                          # fast re-plot: reuse the 91 GB-derived matrix (sRNA-on-TE summed per family x strain)
    E=pd.read_csv(E_csv,index_col=0).reindex(index=M.index,columns=CANON).fillna(0.0)
else:
    famexpr=defaultdict(lambda: defaultdict(float))
    F=f"{ROOT}/results/TEtranscriptCount/all_featureCounts_TE.tab"   # 91 GB: Geneid="name family strain#1#chrom coords"; c[1]=PanSN Chr -> split('#')[0]=strain; c[6]=count
    if os.path.exists(F):
        for ln in open(F):
...
# vibrant single-hue ("one scale") colormaps, Nature-Genetics style, pale -> vivid (not dark/black at top)
CMAP_COUNT=mc.LinearSegmentedColormap.from_list("vivBlue",["#eaf3fb","#9ecae8","#3a8fd4","#1565a8"]); CMAP_COUNT.set_bad("white")
CMAP_EXPR =mc.LinearSegmentedColormap.from_list("vivMag", ["#fdeaf3","#f59ac8","#dd4a9c","#a01a6d"]); CMAP_EXPR.set_bad("#f0f0f0")
def _txtcol(norm): return "white" if norm>0.55 else "#222"   # legible label on a pale->vivid single-hue cell
_Mlog=np.log10(M.values+1); _Mvmax=_Mlog.max()
im=axH.imshow(np.ma.masked_where(M.values==0,_Mlog),aspect="auto",cmap=CMAP_COUNT,vmin=0,vmax=_Mvmax)
...
for i in range(len(M)):
    for j in range(len(CANON)):
        v=int(M.values[i,j])
        if v>0: axH.text(j,i,f"{v:,}" if v<1000 else f"{v//1000}k",ha="center",va="center",fontsize=4.6,color=_txtcol(np.log10(v+1)/_Mvmax if _Mvmax else 0))
axH.set_title("A  TE family of origin — strain-private piRNAs (counts, log)",fontsize=9.6,fontweight="bold",loc="left")
...
_Elog=np.log10(E.values+1); _Evmax=_Elog.max() if _Elog.max()>0 else 1
_Evmin=_Elog[E.values>0].min() if (E.values>0).any() else 0   # start at the data min: the narrow high range (log10 ~4.9-7.9) then spreads across the ramp -> not all dark
imE=axE.imshow(np.ma.masked_where(E.values<=0,_Elog),aspect="auto",cmap=CMAP_EXPR,vmin=_Evmin,vmax=_Evmax)
...
for i in range(len(E)):
    for j in range(len(CANON)):
        ...
        if v>0:
            ...
            axE.text(j,i,lab,ha="center",va="center",fontsize=4.0,color=_txtcol((np.log10(v+1)-_Evmin)/(_Evmax-_Evmin) if _Evmax>_Evmin else 0.5))
axE.set_title("C  TE-family small-RNA expression (sRNA-on-TE, summed, log)",fontsize=9.6,fontweight="bold",loc="left")
