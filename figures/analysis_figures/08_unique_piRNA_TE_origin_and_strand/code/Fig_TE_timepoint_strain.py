...
cmap = LinearSegmentedColormap.from_list("vivBlue",["#eaf3fb","#9ecae8","#3a8fd4","#1565a8"]); cmap.set_bad("white"); heataxes = []   # vibrant single-hue (one scale), pale->vivid
for j, tp in enumerate(TPS):
    ...
    for i in range(len(topfam)):
        for k in range(len(CANON)):
            v = int(Mt.values[i, k])
            if v > 0: axh.text(k, i, f"{v:,}" if v < 1000 else f"{v // 1000}k", ha="center", va="center", fontsize=3.8, color="white" if (np.log10(v + 1)/vmax if vmax else 0) > 0.55 else "#222")
