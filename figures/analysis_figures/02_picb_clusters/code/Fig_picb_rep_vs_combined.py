...
d=d[d.complete].copy()   # data-integrity: defensive filter for concurrency-corrupted PICB runs (complete=False = missing chromosomes); all current runs are complete=True, so no rows are dropped
