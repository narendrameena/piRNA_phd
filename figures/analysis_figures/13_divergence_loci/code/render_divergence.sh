...
tail -n +2 pangenome_te/divergence_loci_final.tsv | while IFS=$'\t' read g39c g39s g39e breadth carrier maxfpm expressing genomen; do
  [ -z "$g39c" ] && continue
  n=$((n+1))
  slug="chr${g39c}_$((g39s/1000000))p$(((g39s%1000000)/1000))_{TOP}"   # {TOP} -> resolved by make_pav_locus_multi.py to the strain SHOWN in Panel C (top expresser after the coverage filter), so filename == panel strain (was ${carrier}, the max-FPM carrier, which can lack in-window reads)
