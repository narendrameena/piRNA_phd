...
# graph_check_pav3.tsv is SHARED and was later appended with theme-24 private loci; restrict the 'shared subset'
# tier to the ORIGINAL 19 loci (shared_subset_loci.rep_cid) so this figure stays reproducible after that append.
_ORIG=set(loci.rep_cid.astype(str))
gp=gp[gp.name.astype(str).str.startswith(("GRCM39FRAME","CTRL_"))|gp.name.astype(str).isin(_ORIG)].copy()
...
_nsub=int((gp.tier=="shared\nsubset").sum())
axC.text(0.5,0.52,f"controls validate the GRCm39 group (≈1.0);\nthe {_nsub} shared-subset loci = 0.0\n→ GENUINELY absent from GRCm39's path",transform=axC.transAxes,ha="center",fontsize=6.9,color="#B2182B",fontweight="bold")
...
# --- per-figure SourceData (frozen: only this figure's values; immune to graph_check_pav3 appends) ---
sd=[]
for k,v in vc.items(): sd.append(dict(panel="A_sharing",key=f"{k}_strains",metric="n_distinct_loci",value=int(v)))
sd.append(dict(panel="A_sharing",key="(summary)",metric="n_strain_entries",value=int(len(ent))))
sd.append(dict(panel="A_sharing",key="(summary)",metric="n_distinct_loci",value=int(len(loci))))
for f_,v in fam.items(): sd.append(dict(panel="B_TEfamily",key=str(f_),metric="n_strain_entries",value=int(v)))
sd.append(dict(panel="B_TEfamily",key="(median)",metric="median_te_div_pct",value=round(float(ent.te_div.median()),1)))
for _,r in gp.iterrows(): sd.append(dict(panel="C_graphPAV",key=str(r["name"]),metric=str(r["tier"]).replace("\n"," ")+"_GRCm39_cov",value=round(float(r["GRCm39"]),3)))
for _,r in dels.iterrows(): sd.append(dict(panel="D_toploci",key=f"chr{r.chrom}:{int(r.start)}",metric=str(r.te_family)[:30],value=round(float(r.uniqFPM),1)))
pd.DataFrame(sd).to_csv(f"{D}/source_data/SourceData_Fig_shared_subset.csv",index=False)
out=f"{T}/figures/Fig_shared_subset"
for e in ("pdf","svg","png"): fig.savefig(f"{out}.{e}",bbox_inches="tight")
print("wrote",out,"| subset-tier loci",_nsub,"(expect 19)")
